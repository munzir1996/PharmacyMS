...
<script>
    ...
    $("#addModal").click(function(){
		...
		$("#dynamicFormModal").append("<div class='col-12 row removeSaleModal' id='test'><div class='col-5'><div class='form-group'><label>Product <span class='text-danger'>*</span></label><select class='select2 form-select form-control' name='sales["+i+"][product]'><option disabled selected > Select Product</option>@foreach (\App\Models\Product::with(['purchase',])->whereHas('purchase', function($query) { $query->where('quantity', '>', 0); })->get() as $product)<option value='{{$product->id}}'>{{$product->purchase->product}} -- {{AppSettings::get('app_currency', '$')}}{{$product->discountedPrice}}</option>@endforeach</select></div></div><div class='col-5'><div class='form-group'><label>Quantity</label><input type='number' value='1' class='form-control' name='sales["+i+"][quantity]' required></div></div><div class='col-2 align-self-center'><button type='button' class='btn btn-danger remove-tr'>Remove</button></div></div>");
		$('.select2').select2();
	});

    $(document).on('click', '.remove-tr', function(){
		$(this).closest('.removeSaleModal').remove();
    });
</script>
<!-- Page JS -->
@stack('page-js')
